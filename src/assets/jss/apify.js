...
function getUpWorkReviews(url, DID) {
  ...
  syncActor(actorUrl, postData, DID);
}

function syncActor(url, postData, DID) {
  fetch(url, {
    method: 'POST',
    ...
    .then(function(data) {
      apifyExecution = data;
      apifyResults = data;
      storeResults(data,DID);
    });
}

function getTripAdvisorReviews(url, DID) {
  ...
  startCrawler(crawlerUrl, postData, DID);
}

function getYelpReviews(url, DID) {
//  var url = $("#yelpURL").val();
  console.log(url);
  if ((typeof url === 'undefined') || url.length == 0) {
    return false;
...
function startCrawler(crawlerUrl, postData, DID) {
  fetch(crawlerUrl, {
    method: 'POST',
    body: JSON.stringify(postData),
    headers: {
...
function storeResults(reviews, DID){
